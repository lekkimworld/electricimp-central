...
eventing.queues.motion.subscribe((payload, callback) => {
	...
	oauth().then(data => {
		...
		conn.sobject(process.env.PLATFORM_EVENT_NAME).create(event_payload).then(sf_result => {		
			// post to topic
			eventing.topics.events.publish("success.event.motion", `Sent Platform Event to Salesforce`);
			if (process.env.SF_CHATTER_MENTION_USERID && process.env.SF_CHATTER_MENTION_BODYTEXT && payload.hasOwnProperty("movement") && payload.movement === false) {
			...
			// post to topic
			eventing.topics.events.publish("error.event,motion", `Could not send Platform Event to Salesforce (${err.message})`);
